export function parseQuestions(rawText, limit = 250){
  ...
  const blocks = splitByQuestionNumber(text);
  const questions = [];
  let subject = "General";
  for(const lines of blocks){
    ...
    for(const line of lines){
      ...
      if(/^(EVS|GK|DSC|TET|BANK|RRB|SI|Psychology|Telugu|English)$/i.test(clean)){ subject = clean; continue; }
      usable.push(line);
    }
...
function splitByQuestionNumber(text){
  ...
  for(const line of lines){
    if(/^\d+\s*[\.)]\s+/.test(line)){ if(current.length) blocks.push(current); current = [line]; }
...
function parseBlock(lines, subject){
  ...
  let text = lines.join("\n").trim(), answer = null;
  const answerMatch = text.match(/Answer\s*[:：]?\s*([ABCD])/i);
  if(answerMatch) answer = "ABCD".indexOf(answerMatch[1].toUpperCase());
  text = text.replace(/^Answer\s*[:：]?\s*[ABCD].*$/gim,"").trim();
  const optionRegex = /^\s*([ABCD])\s*[\.)]\s*(.*)$/gmi;
...
function formatStatements(q){
  return String(q || "").replace(/\s+(I{1,3}|IV|V)\.\s+/g,"\n$1. ").replace(/\s*(\([ivxlcdm]+\)|[ivxlcdm]+\.)\s*/gi,"\n$1 ").replace(/\n{2,}/g,"\n").trim();
}
export function expectedCount(raw){
  const nums = [...normalize(raw).matchAll(/(?:^|\n)\s*(\d+)\s*[\.)]\s+/g)].map(m=>Number(m[1])).filter(n=>n>0&&n<=500);
